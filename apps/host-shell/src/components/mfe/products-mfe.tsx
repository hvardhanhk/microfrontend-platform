...
import {
  Card, CardBody, CardHeader, CardFooter, Button, Input, Badge,
  Pagination, Tabs, TabList, Tab, Dropdown,
} from '@platform/ui';
...
const MOCK_PRODUCTS: Product[] = Array.from({ length: 12 }, (_, i) => ({
  id: `prod_${i + 1}`,
  name: ['Wireless Headphones', 'Laptop Stand', 'Mechanical Keyboard', 'USB-C Hub',
    'Monitor Light Bar', 'Ergonomic Mouse', 'Desk Mat', 'Webcam HD',
    'Noise Cancelling Earbuds', 'Portable Charger', 'Smart Speaker', 'Cable Organizer'][i],
  slug: `product-${i + 1}`,
  description: 'Premium quality product designed for professionals. Built with sustainable materials.',
  price: [79.99, 49.99, 149.99, 39.99, 59.99, 69.99, 29.99, 89.99, 129.99, 44.99, 99.99, 19.99][i],
  currency: 'USD',
  images: [{ id: `img_${i}`, url: `https://picsum.photos/seed/${i + 10}/400/400`, alt: `Product ${i + 1}`, width: 400, height: 400, isPrimary: true }],
  category: { id: `cat_${(i % 3) + 1}`, name: ['Audio', 'Accessories', 'Peripherals'][i % 3], slug: ['audio', 'accessories', 'peripherals'][i % 3] },
  tags: ['featured'],
  variants: [],
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex gap-3">
          <Input placeholder="Search products..." value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} className="w-64" />
          <Dropdown
      ...
      <Tabs defaultIndex={0} onChange={(i) => { setSelectedCategory(CATEGORIES[i]); setCurrentPage(1); }}>
        <TabList>
          ...
          <Card key={product.id} className="group transition-shadow hover:shadow-lg">
            <CardHeader className="p-0">
              <div className="relative aspect-square overflow-hidden rounded-t-lg bg-gray-100 dark:bg-gray-800">
                <img src={product.images[0]?.url} alt={product.name} className="h-full w-full object-cover transition-transform group-hover:scale-105" loading="lazy" />
              </div>
            </CardHeader>
                ...
                if (cartItem) {
                  return (
                    <div className="flex items-center gap-2">
                      ...
                      <Button size="icon" variant="outline" onClick={() => updateItemQuantity(product.id, cartItem.quantity + 1)}>+</Button>
                    </div>
                  );
                }
                return <Button size="sm" onClick={() => handleAddToCart(product)}>Add to Cart</Button>;
              })()}
            </CardFooter>
          </Card>
        ))}
