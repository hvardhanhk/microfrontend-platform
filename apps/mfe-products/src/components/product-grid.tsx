...
export function ProductGrid() {
  ...
  const handleAddToCart = (product: Product) => {
    ...
    EventBus.publish('notification:show', { type: 'success', message: `${product.name} added to cart` });
  };
        ...
        {paginated.map((product) => (
          <Card key={product.id} className="group hover:shadow-lg transition-shadow">
            <div className="aspect-square overflow-hidden rounded-t-lg bg-gray-100">
              <img src={product.images[0]?.url} alt={product.name} className="h-full w-full object-cover group-hover:scale-105 transition-transform" loading="lazy" />
            </div>
            <CardBody>
              <h3 className="font-semibold text-gray-900 dark:text-white">{product.name}</h3>
              <p className="text-sm text-gray-500">{product.category.name}</p>
              <Badge variant="default" className="mt-1">{product.rating.average.toFixed(1)} ★</Badge>
            </CardBody>
